public class Main{
  //Arraylist de objeto SeleccionFutbol. Idependientemente de la clase hija a la que pertenezca el objeto 
    public static ArrayList<SeleccionFutbol> integrantes = new ArrayList<SeleccionFutbol>();
    
    public static void main(String[] args){
                  SeleccionFutbol delBosque = new Entrenador(1, "Vicente, "Del Bosque", 60,28489);
                  SeleccionFutbol iniesta = new Futbolista(2,"Andres". "Iniesta",29, 6,"Interior Derecho");
                  SeleccionFutbol raulMartinez = new Masajsita(3, "Raul",",Martinez",41,"Licenciado en Fisioterapia", 18);
                  ...
                  for (SeleccionFutbol integrante : integrante) {
                      System.out.print(integrante.getNombre()+ " "+integrante.getApellido()+ "->");
                      integrante.viajar();
                   }
                   //ENTRENAMIENTO
                   System.out.println("nEntrenamiento:Tods los inegrantes tienen su funcion en un entrenamiento (Especializacion)");
                   for (SeleccionFutbol integrante : integrante){
                      System.out.print(integrante.getNombre() + " " +integrante.getApellidos()+ " -> ");
                      integrante.partidoFutbol();
                   }
                   //PlANIFICAR ENTRENAMIENTO
                   System.out.println("nPlanificar Entrenamiento: Solo el entrenador tiene el metodo para planificar un entranamiento:");
                   System.out.print(delBosque.getNombre() + " " + delBosque.getApellidos() + " -> ");
                   ((Entrenador) delBosque).planificarEntrenamiento();
                    
                    //ENTREVISTA 
                    System.out.println("nEntrevista: Solo el futbolista tiene el metodo para dar una entrevista:");
                    Sytem.out.print(iniesta.getNombre() + " "+iniesta.getApellidos() + " -> ");
